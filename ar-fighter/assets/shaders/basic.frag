...
void main()
{
    ...
    if(num_directional_lights > 0)
    {
        ...
        // Diffuse
        mediump float diffuseStrength = max(dot(Normal, directional_lights[0].direction), 0.0);
        
        // Specular
        mediump vec3 halfwayDir = normalize(directional_lights[0].direction + viewDir);
        mediump float specularStrength = pow(max(dot(Normal, halfwayDir), 0.0), material.shininess);
        
        // Calculate colours
        mediump vec4 ambient = directional_lights[0].ambient * material.diffuse * ambientStrength;
        mediump vec4 diffuse = directional_lights[0].diffuse * material.diffuse * diffuseStrength;
        mediump vec4 specular = directional_lights[0].specular * material.specular * specularStrength;
